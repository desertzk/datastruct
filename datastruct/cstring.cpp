...
#ifdef __cplusplus
vector<string> splite(string src,string regexstr)
{
...
{
	char *pneedchange = str+ start;
	int remain=strlen(pneedchange);
	int movecount = remain - num;
	for (int i = 0; i < movecount; ++i)
		pneedchange[i] = pneedchange[i + num];
	str[start + movecount] = 0;
	return str;
}
...
//可能隐含越界问题
int end_with(char *str,const char *end)
{
	char *tmp = str;
...
int main1213()
{
	...
	const char *str = ",,dga,sfga,sfda,,,111,";
	//test splite
#ifdef __cplusplus
	vector<string> v = splite(str, ",");
	auto it = v.begin();
	while (it != v.end())
	{
		printf("%s\n", it->c_str());
